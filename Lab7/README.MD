...
Compare with the result obtained in 8.1. You may find that under stepping load situation, the performance of open-loop converter is quite “acceptable” since the output voltage changes only about 1V. Aside from the fact that 1V change is not always acceptable, in the realistic circuit, the voltage deviation will be higher. Explain why. (Hint: Although the circuit looks quite “realistic”, it is actually not. Think about what is missing in the schematic.)  Component variations with tolerances that deviate from the nominal value.  Dynamic response limitations unding changing conditions can result in larger voltage deviations. Nonideal components.  Noise and disturbances.  In open system lack of feedback mechanism to adjust changes in voltages in response to the changes in load. [chat.pdf]
